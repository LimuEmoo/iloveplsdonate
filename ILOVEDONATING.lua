...
local win = Flux:Window("CapySploit", "Cracked by heqds#2146", Color3.fromRGB(6, 137, 189), Enum.KeyCode.RightShift)
...
webhookTab:Button("Test", "Sends a test message to your webhook to verify it is working", function()
    if getgenv().settings.webhookBox then
        webhook("CapySploit PLS DONATE AFK Script")
...
otherTab:Button("Free Robux", "This gives free robux (not client sided) I found this glitch scrolling through youtube.", function()
    local player = game:GetService("Players").LocalPlayer
    player:Kick("You thought")
end)

otherTab:Button("Fake Nuke", "This makes a fake nuke happen on your server.\nOnly you can see this.", function()
local player = game:GetService("Players").LocalPlayer
_G.WhoDonated = "haz3mn"
_G.HowMuchRobux = 100000
_G.WhoGotRobux = player.Name
    ...
    if getgenv().settings.webhookToggle and getgenv().settings.webhookBox then
        task.spawn(function()
            ...
            if not sent then
                ...
                webhook(tostring("💰 You just stole and laundered "..amount.." robux from a little african child using "..Players.LocalPlayer.DisplayName.." *[After Laundering Fees: "..math.floor(amount * 0.6).."]* (Total laundered robux: " ..newRaised..")"))
            end
            RaisedC = newRaised
